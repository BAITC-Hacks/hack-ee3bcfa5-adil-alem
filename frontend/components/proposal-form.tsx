...
export function ProposalForm({ id }: { id: string }) {
  ...
  const storageKey = `sana-proposal-${id}`;
  useEffect(() => {
    try {
      const saved = JSON.parse(sessionStorage.getItem(storageKey) || "null");
      setDraft(saved?.draft && Object.values(saved.draft).every(value => typeof value === "string") ? saved.draft : {});
      setSubmitted(saved?.submitted === true);
    } catch { /* Storage is optional; server submissions remain authoritative. */ }
    setRestored(true);
  }, [storageKey]);
  function preserve(values: Record<string, string>, sent = false) {
    setDraft(values);
    try { sessionStorage.setItem(storageKey, JSON.stringify({ draft: values, submitted: sent })); } catch { /* Browser storage may be unavailable. */ }
  }
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api.submit(id, { team_id: team, ...fields }); preserve({}, true); setSubmitted(true); }
  ...
  if (!restored) return <LoadingState />;
  if (submitted) return <div className="state" role="status"><h1>Proposal submitted</h1><button className="button secondary" disabled={status !== "ready"} onClick={() => { preserve({}); setSubmitted(false); }}>Write another proposal</button><p>Your original proposal was submitted for a manual business decision. This is a saved submission receipt, not its current decision status.</p><Link className="button" href={`/challenges/${id}`}>Back to challenge</Link><Link className="text-button" href="/">Explore challenges</Link></div>;
  if (status === "loading") return <LoadingState />;
  if (!data || status !== "ready") return <ErrorState notFound={status === "missing"} retry={retry} />;
  const [task, teams] = data;
  return <><Link className="back-link" href={`/challenges/${id}`}>← Back to challenge</Link><header className="detail-header"><div className="eyebrow accent">STUDENT PROPOSAL</div><h1>Turn your idea into a proposal.</h1><p className="muted">Share your approach in your own words. The business makes the final decision.</p></header><div className="detail-layout"><form onSubmit={submit} onChange={event => preserve(Object.fromEntries(Array.from(new FormData(event.currentTarget).entries(), ([key, value]) => [key, String(value)])))} className="brief proposal-form"><h2>Your original proposal</h2><p className="muted text-sm">Your submission is shared with the business as written.</p><fieldset disabled={sending || !teams.length}><label>Team<select aria-label="Team" name="team" defaultValue={draft.team || ""} required><option value="" disabled>Select your team</option>{teams.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label>{!teams.length && <p role="status">No teams are available yet. A team must be added before you can submit.</p>}<label>Solution idea<textarea name="solution_idea" aria-label="Solution idea" defaultValue={draft.solution_idea || ""} required rows={5} /></label><label>Implementation plan<textarea name="plan" aria-label="Implementation plan" defaultValue={draft.plan || ""} required rows={5} /></label><label>Timeline<input name="timeline" aria-label="Timeline" defaultValue={draft.timeline || ""} required placeholder="For example: three weeks, with a demo in week two" /></label><label>Prototype URL<input name="prototype_url" aria-label="Prototype URL" defaultValue={draft.prototype_url || ""} required type="url" placeholder="https://example.com/your-prototype" /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="button" type="submit">{sending ? "Submitting…" : "Submit proposal"}</button></fieldset></form><aside className="readiness-panel"><div className="eyebrow">YOUR CHALLENGE</div><h2 className="reference-title">{task.title}</h2><ReadinessScore score={task.score} level={task.readiness_level} /><ReadinessBadge level={task.readiness_level} />{([ ["expected_result", "Expected result"], ["constraints", "Known constraints"] ] as const).map(([field, label]) => <section className="reference-field" key={field}><h3>{label}</h3><p>{task[field] || "Not confirmed yet"}</p>{task[field] && !task.confirmed_fields.includes(field) && <span className="unconfirmed-label">Not confirmed yet</span>}</section>)}<p className="panel-description">Every readiness level is open for proposals. Unknown information is an opportunity to ask questions.</p></aside></div></>;
}
